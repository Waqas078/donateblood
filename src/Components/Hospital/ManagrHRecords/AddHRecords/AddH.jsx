...
function AddD({placeholder,data}) {
  ...
  const handleClick =(props) => {
    ...
    setWordEntered(props.phoneNumber);
    setTrigger(false);
  };
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    try {
      console.log({person, group, number, phone, name, action});
      const { data } = await createRecord({person, group, number, phone, name, action});
      console.log(data);
    } catch (error) {
      console.log(error);
    }
                            ...
                            {filteredData.length !== 0 && (
                              <div className="absolute top-[17rem] rounded-br-lg rounded-bl-lg right-[75px] h-auto my-1 mx-7 w-[74%] mb-1 border border-blue-300 bg-[#E8F6FE] ">
                                ...
                                { trigger === true ?  filteredData.slice(0, 15).map((user) => (
                                  <button key={user.phoneNumber} onClick={()=>{handleClick(user)}}
